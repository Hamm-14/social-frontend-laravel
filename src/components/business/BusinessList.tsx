...
import { useGetAllBusinessesQuery } from "../../apis/business";
...
const BusinessList = () => {
  ...
  const { data, isLoading, isSuccess } = useGetAllBusinessesQuery("");
              ...
              {!isLoading && isSuccess && data.length > 0 && (
                <TableBody>
                  {(rowsPerPage > 0
                    ...
                    : data
                  ).map((row: UserBusiness) => (
                    <StyledTableRow
                      key={row._id}
                      style={{ cursor: "pointer" }}
                      onClick={() => handleViewClick(row._id)}
                    >
                      <StyledTableCell>{row._user._firstName}</StyledTableCell>
                      <StyledTableCell component="th" scope="row">
                        {row._user._lastName}
                      </StyledTableCell>
                      <StyledTableCell>{row._businessName}</StyledTableCell>
                      <StyledTableCell align="center">{row._businessPhoneNumber}</StyledTableCell>
